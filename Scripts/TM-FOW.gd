...
func switchToPlayer(player_id):
	for x in range(width):
		for y in range(height):
			set_cell(x, y, player_visibilities[player_id][x][y])
...
func getPlayerVisibleTiles(player_id):
	var return_tiles = []
	var player_armies = adventure_map.army_instances[player_id]
...
func updatePlayerVisibility(player_id, army_id):
	...
	var army = adventure_map.army_instances[player_id][army_id]
